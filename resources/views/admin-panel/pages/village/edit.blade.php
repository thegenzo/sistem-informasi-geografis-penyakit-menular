        ...
        <section class="section">
            <div class="section-header">
                ...
                <div class="section-header-breadcrumb">
                    ...
                    <div class="breadcrumb-item">Data Kelurahan</div>
                    <div class="breadcrumb-item active">Edit Kelurahan</div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12 col-md-6 col-12 col-sm-6">
                    ...
                    @endif
                    <form action="{{ route('admin-panel.villages.update', $village->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="district_id" id="district_id" class="form-control select2">
                                        ...
                                        @foreach (\App\Models\District::all() as $data)
                                            <option value="{{ $data->id }}" {{ $data->id == $village->district_id ? 'selected' : '' }}>{{ $data->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="name">Nama Kelurahan <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" name="name" id="name" value="{{ $village->name }}">
                                </div>
                                <div id="map"></div>
                                <input type="hidden" name="coordinates" id="coordinates" value="{{ $village->coordinates }}">

                            </div>
                        </div>
                        <a href="{{ route('admin-panel.villages.index') }}"
